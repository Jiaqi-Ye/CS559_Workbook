 ...
 * @property {number} [width=300] - panel width in pixels
 * @property {InputHelpers.WhereSpec} [where] - explicit container (overrides world default)
 * @property {number} [widthdiv=1] - panel width subdivision for multi-column layouts
 * @property {boolean} [adjusted=false] - when true, lets lil-gui auto-size labels instead of forcing panel width
 * @property {boolean} [useLilGUI=true] - true for lil-gui controls, false for `LabelSlider` controls
 * @property {"inline-block"|"inline"|string} [labelDisplay="inline-block"] - slider-label display mode in `LabelSlider` mode (`useLilGUI:false`): `"inline-block"` is the default stacked look, `"inline"` enables compact single-row sliders
 * @property {"where"|"floating"|"canvas-overlay"} [guiPlacement="canvas-overlay"] - lil-gui placement strategy (used only when `useLilGUI:true`)
 */
 ...
 *
 * Compact mode note:
 * - The default `labelDisplay` is `"inline-block"` (not compact).
 * - For compact sliders, set `useLilGUI: false` and `labelDisplay: "inline"`.
 *
 * Typical conversion:
 *
 * Old:
