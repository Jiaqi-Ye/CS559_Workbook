 ...
 * @property [webgpu=true] - `true` if you want to use the WebGPURenderer and `false` if you want to use the WebGLRenderer
...
export class GrWorld {
     ...
     * 
     * This constructor function is private and should not be used directly.
     * Instead, use `await GrWorld.new()` within an async context.
     * 
     * @param {Symbol} priv - Private constructor symbol to ensure this constructor is not constructed outside the constructor `new` function.
     * @param {GrWorldProperties} params
     */
    constructor(priv, params) {

        if (priv !== PRIVATE_CONSTRUCTOR) {
            throw new Error("GrWorld must be constructed with `await GrWorld.new()`; it cannot be constructed directly (`await GrWorld.new()`)");
        }
        ...
        /** @type T.WebGLRenderer | WebGPURenderer */
        this.renderer;
        if ("renderer" in params) {
            this.renderer = params.renderer;
        } else if (params.webgpu !== false) {
     ...
     * 
     * This function is async and therefore must be awaited
     * if code after it depends on the new `GrWorld`.
     * 
     * ## Example Use
     * ```
     ...
     * ```
     * 
     * @param {GrWorldProperties} params The parameters for constructing the GrWorld.
     * @returns {Promise<GrWorld>} Returns a await GrWorld.new depending on the params.
     */
    static async new(params = {}) {
        const out = new GrWorld(PRIVATE_CONSTRUCTOR, params);

        // Initialize the WebGPU Renderer
        const renderer = out.renderer;
        if (!("renderer" in params) && (renderer instanceof WebGPURenderer)) {
            //@ts-ignore
            await renderer.init();
        }
