...
const { box, world } = await makeDemoWorld("wood2a", "TSL Wood 2A");
...
const angle = nv.atan2(nu);

const d = dist.add(angle.mul(radialFreq).sin().mul(radialAmount));
const su = d.fract();
const dst = su.sub(0.5).abs().add(angle.mul(thickFrequency).sin().mul(tickAmount));
