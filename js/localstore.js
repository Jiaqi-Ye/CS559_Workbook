...
function getSheetsStatusSpans() {
    const spans = document.querySelectorAll('.sheets-status-span');
    if (spans.length > 0) return spans;

    const fallback = document.getElementById('sheets-status-span');
    return fallback ? [fallback] : [];
}

function setSheetsStatus(message, color) {
    const spans = getSheetsStatusSpans();
    spans.forEach(span => {
...
function showFirstVisitMessage() {
    ...
    if (!window.hasPageBeenVisited() && !WORKBOOK_CONFIG.solution) {
        ...
        columns.forEach(col => {
            ...
            div.classList.add('first-visit-warning');
...
window.submitSheet = async function(event) {
    ...
    setSheetsStatus(`Saving... (${submitType})`, "blue");
    ...
    if (!netID) {
        setSheetsStatus("❌ Missing NetID. Please fill in your NetID before submitting.", "red");
        setStoredValue("sheets-submitted", "error", false);
        return;
    }
    if (!githubID) {
        setSheetsStatus("❌ Missing GitHub ID. Please fill in your GitHub ID before submitting.", "red");
        setStoredValue("sheets-submitted", "error", false);
        return;
    }
    if (!ainote && (submitType === 'submit')) {
        setSheetsStatus("❌ Missing AI Disclosure. Please fill in your AI disclosure before submitting.", "red");
        setStoredValue("sheets-submitted", "error", false);
        return;
    }
    const payload = {
    ...
    if (result.status === 'success') {
        setSheetsStatus(`✅ Workbook Data Submitted Successfully! (${submitType}), does not include GitHub push!`, "green");
        ...
        setSheetsStatus("❌ Error: " + result.message, "red");
        setStoredValue("sheets-submitted", "error", false);
        if (submitType === 'submit') {
        ...
        setSheetsStatus("❌ Network Error. Please check your internet connection.", "red");
        setStoredValue("sheets-submitted", "error", false);
        if (submitType === 'submit') {
            setStoredValue("sheets-final", "error", false);
        }
...
function updateSubmissionSatus() {
    ...
    if (statusDiv) {
        setSheetsStatus("not submitted (according to local storage)");
        const submissionStatus = getStoredValue(checkKey, false);
        if (submissionStatus === "success") {
            setSheetsStatus("✅ Previously submitted successfully. (at " + getStoredValue("sheets-response-time", false) + ")", "green");
            updateSubmitButtonSuccess(submitButton);
        } else if (submissionStatus === "error") {
            const failTime = getStoredValue("sheets-response-time", false);
            setSheetsStatus("❌ Previous submit attempt failed. (at " + (failTime ? failTime : "no time given") + ")", "red");
        }
    }
};
