...
import { addLabelToGrObject } from "CS559-Framework/label.js";
...
class Flat extends GrObject {
    constructor(params={}) {
        ...
        addLabelToGrObject(this, label);

        if (helper) {
            let helpobj = new VertexNormalsHelper(mesh);
            this.objects.push(/** @type {T.Object3D} */ (/** @type {unknown} */ (helpobj)));
        }
    }
}
...
class MultiFlat extends GrObject {
    constructor(params={}) {
        ...
        let helper = params.helper || false;
        let label = params.label ?? "MultiFlat";
        //
        // make regular arrays, then convert to buffers later
        let verts = []
        let normals = [];
...
world.add(new Flat({x: 6, bump:true, helper:true, label:"Bump Map"}));
world.add(new Flat({x: 3, normalMap:true, helper:true, label:"Normal Map"}));
world.add(new Flat({x: 0, label:"flat plane"}));
world.add(new MultiFlat({x: -3, forward:.35, zeroz:true, helper:true, label:"using normals"}))
world.add(new MultiFlat({x: -6, forward:.35, helper:true, label:"real object"}));
